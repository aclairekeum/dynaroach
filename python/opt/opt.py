...
class Opt(object):
  ...
  def pars( self,fi='',p0=env,x=None,**p ):
    """
    ...
    if x is None:
      x = self.x
    return dict([(var,(x[self.j[i]:self.j[i+1]])) 
  ...
  def unpack(self, x):
    ...
    if np.asarray(x).shape == ():
      x = np.asarray([x])
    for i,var in enumerate(self.p['opt']):
  ...
  def __repr__( self ):
    ...
    """
    return 'f(x) = {0.2f}; x = {!s}'.format( self.f(self.x), self.x )
